...
public class LessonsActivity extends AppCompatActivity {
    ...
    private void checkProgressAndProceed() {
        final boolean[] allLessonsCompleted = {true}; // Use an array to make it effectively final
        for (int i = 1; i <= lessonCount; i++) {
            String lessonKey = "1_" + i;
            DatabaseReference userLessonRef = database.child("Users").child("UserID").child("Lessons").child(lessonKey);
            int finalI = i; // Store the value of 'i' for use in the inner class
            userLessonRef.addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
                    Integer counter = dataSnapshot.child("counter").getValue(Integer.class);
                    if (counter == null || counter < 5) {
                        allLessonsCompleted[0] = false; // Update the array element
                        Toast.makeText(LessonsActivity.this, "Complete all characters to 5/5 before proceeding. Next session is in 3 hours", Toast.LENGTH_LONG).show();
                    } else {
                        // If all lessons have been checked, proceed to MainActivity
                        if (allLessonsCompleted[0] && finalI == lessonCount) {
                            startActivity(new Intent(LessonsActivity.this, HomeFragment.class));
                        }
                    }
                }
